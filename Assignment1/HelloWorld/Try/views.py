...
def get_all_tweets_imageurls(screen_name, Num = 100):    
...
if __name__ == '__main__':
    ...
    imageurls,filenames = get_all_tweets_imageurls(input(""),100)
...
def search(request):
    ...
    if request.POST:
        ...
        imageurls,filenames = get_all_tweets_imageurls(name,100)
        download_image(imageurls,filenames)
        convert_image_video(filenames,'output.mp4')
        #change this environment variable to yours
        vision_analysor(imageurls,filenames,'1.json')
        print("All done!")
